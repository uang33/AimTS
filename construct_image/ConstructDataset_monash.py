...
from monash_cls import load_from_tsfile_to_dataframe
...
class TSRegressionArchive(BaseData):
    ...
    def load_single(self, filepath):
        ...
        if self.config['task'] == 'regression':
            ...
            labels_df = pd.DataFrame(labels.cat.codes, dtype=np.int32)  # int8-32 gives an error when using nn.CrossEntropyLoss
        else:  # e.g. imputation
            try:
